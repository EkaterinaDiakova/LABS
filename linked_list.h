...
public:
		...
		class Iterator
		    ...
		    private:
		        Node *head;
		    public:
		        Iterator(Node* m_head) { this->head = m_head; }
		        Iterator operator++() { Iterator itr = *this; head = head->next; return itr; }
		        T& operator*() { return head->value; }
		        bool operator==(const Iterator &itr) { return head->value == itr.head->value; }
		        bool operator!=(const Iterator &itr) { return head->value != itr.head->value; }
		    };
