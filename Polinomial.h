...
public:
	...
	polinomial<T,T2>* sum(polinomial<T,T2>& pol_2)
	{
	...
	polinomial<T,T2>* multiplicate(polinomial<T,T2>& pol_2)
	{
	...
	T composition(polinomial<T,T2>* pol_2, T arg)
	{
		T tmp = this->argument(arg);
		tmp = pol_2->argument(tmp);
		return tmp;
	}
